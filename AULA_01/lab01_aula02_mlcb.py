...
#Os resultados foram parcialmente corretos.
#• "Quero consultar quanto dinheiro tenho" → fazer_pix: incorreto. O correto seria consultar_saldo.
#• "Pode me ajudar a fazer um pix?" → fazer_pix: correto.
#• "Gostaria de cancelar meu cartão de crédito" → cancelar_conta: incorreto. A mensagem se refere ao cancelamento do cartão, enquanto o treinamento possui exemplos relacionados ao cancelamento da conta.


# 2 - Detectado algum erro, qual seria a maneira mais correta de melhorar o resultado do algoritmo?

#O algoritmo possui poucos exemplos para cada intenção. Quanto mais exemplos diferentes forem inseridos, melhor conseguirá identificar os padrões das mensagens.


# 3 - Detalhe a função do LogisticRegression no algorítmo.

#O LogisticRegression é o modelo responsável por realizar a classificação das mensagens.
